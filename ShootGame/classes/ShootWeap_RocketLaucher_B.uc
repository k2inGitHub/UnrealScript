 ...
 */
class ShootWeap_RocketLaucher_B extends ShootWeap_ShockRifleBase;
...
/*
	...
	*/
	...
	ProjectileFire_F();
	SetTimer(0.5,false,'ProjectileFire_E');
	SetTimer(1,false,'ProjectileFire_D');
	SetTimer(1.5,false,'ProjectileFire_C');
...
function ProjectileFire_Circle(){
		...
		local int i;
        local rotator MyRot;
        local ShootProj_Rocket MyShockBall;
		local vector		RealStartLoc;
        ...
        {
			MyRot.Yaw += 3276*2;
            MyShockBall = spawn(class'ShootProj_Rocket', /*self*/,,RealStartLoc/* Location*/);
			MyShockBall.Instigator=UTPawn(Instigator);
			MyShockBall.Init(Vector(MyRot));
        }
