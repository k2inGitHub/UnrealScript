 ...
 */
class ShootWeap_RocketLaucher extends ShootWeap_ShockRifleBase;
...
ProjectileFire_E();
SetTimer(0.5,false,'ProjectileFire_D');
SetTimer(1,false,'ProjectileFire_C');
SetTimer(1.5,false,'ProjectileFire_B');
...
simulated function ProjectileFire_D(){
        ...
        {
			MyRot.Yaw+=range/(n-1);
            MyShockBall = spawn(class'ShootProj_LinkPlasma_Blue', /*self*/,,(RealStartLoc)/* Location*/);
			MyShockBall.Instigator=UTPawn(Instigator);
			MyShockBall.Init(Vector(MyRot));
            //MyRocket.Init(normal(Enemy.Location - Location)); // To shoot at player location
            }
...
simulated function ProjectileFire_C(){
        ...
        {
			MyRot.Yaw+=range/(n-1);
            MyShockBall = spawn(class'ShootProj_LinkPlasma_Gold', /*self*/,,(RealStartLoc)/* Location*/);
			MyShockBall.Instigator=UTPawn(Instigator); 
			MyShockBall.Init(Vector(MyRot));
            //MyRocket.Init(normal(Enemy.Location - Location)); // To shoot at player location
            }
...
simulated function ProjectileFire_B(){
        ...
        {
			MyRot.Yaw+=range/(n-1);
            MyShockBall = spawn(class'ShootProj_LinkPlasma_Red', /*self*/,,(RealStartLoc)/* Location*/);
			MyShockBall.Instigator=UTPawn(Instigator);
			MyShockBall.Init(Vector(MyRot));
            //MyRocket.Init(normal(Enemy.Location - Location)); // To shoot at player location
            }
...
simulated function ProjectileFire_B(){
		...
		//local UTProj_LinkPlasma Proj_L;
		//local UTProj_Rocket Rocket;
		local vector		RealStartLoc,CamDirX, CamDirY, CamDirZ;
		...
		RealStartLoc = GetPhysicalFireStartLoc();//射击位置
		/*
		MyRot=GetAdjustedAim( RealStartLoc );//射击方向
		`log(RealStartLoc);
		...
		RealStartLoc=RealStartLoc+offset.X*CamDirX+offset.Y*CamDirY+offset.Z*CamDirZ;
		...
		*/
		for(j=0;j<4;j++){
			...
			else
			{
				...
				MyRot.Yaw-=(n/2-0.5)*2048;
			}

			
			
			//MyRot+=rotator(offset);
...
defaultproperties
{
	...
	WeaponFireTypes(1)=EWFT_None//Projectile
